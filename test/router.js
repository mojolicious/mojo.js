 ...
 * /optional/**.txt
 */
r.any('/optional/:foo/:bar', {format: 'txt'}).to({bar: 'test', format: null});
 ...
 * /optional2/**.txt
 */
r.any('/optional2/:foo').to({foo: 'one'}).any('/:bar', {format: 'txt'}).to({bar: 'two', format: null});
...
t.test('Alternatives with default', t => {
  ...
  t.equal(plan.render({format: 'txt'}).path, '/alternatives/11.txt', 'right path');
  t.equal(plan.render({foo: 12}).path, '/alternatives/12', 'right path');
  t.equal(plan.render({foo: 12, format: 'txt'}).path, '/alternatives/12.txt', 'right path');
  ...
  t.equal(r.lookup('alternatives_foo').render({format: 'txt'}), '/alternatives/11.txt', 'right path');
  t.equal(r.lookup('alternatives_foo').render({foo: 12, format: 'txt'}), '/alternatives/12.txt', 'right path');
  t.done();
});
...
t.test('Optional placeholder', t => {
  ...
  t.same(plan.steps, [{foo: 23, bar: 'test', format: null}], 'right structure');
  t.equal(plan.render().path, '/optional/23', 'right path');
  t.equal(plan.render({format: 'txt'}).path, '/optional/23/test.txt', 'right path');
  t.equal(plan.render({foo: 12, format: 'txt'}).path, '/optional/12/test.txt', 'right path');
  const plan2 = r.plot({method: 'GET', path: '/optional/23/24'});
  t.same(plan2.steps, [{foo: 23, bar: '24', format: null}], 'right structure');
  t.equal(plan2.render().path, '/optional/23/24', 'right path');
  t.equal(plan2.render({format: 'txt'}).path, '/optional/23/24.txt', 'right path');
  t.done();
});
