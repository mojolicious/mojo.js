...
The `stash` is a property of the context and is used primarily to pass data to views. While views can be inlined for
single file apps, they are usually kept as separate files in a `views` directory.
...
The default mojo.js template engine is [ejs](https://www.npmjs.com/package/ejs), but any other template system would
work just as well.

## HTTP

The `req` and `res` properties of the context give you full access to all HTTP features and information.
