...
export default class Static {
  ...
  async serveFile (ctx, file) {
    ...
    // Entire file
    if (range === undefined) {
      ctx.res.status(200).length(length).type(type).send(file.createReadStream());
      return;
    }
    ...
    // Range
    ctx.res.status(200).length((end - start) + 1).type(type).set('Content-Range', `bytes ${start}-${end}/${length}`)
