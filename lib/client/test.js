...
export default class TestClient extends MockClient {
  ...
  elementExists (selector) {
    const element = this._domCache().window.document.querySelector(selector);
    this.assert('ok', [element != null], `element for selector "${selector}" exists`);
    return this;
  }

  elementExistsNot (selector) {
    const element = this._domCache().window.document.querySelector(selector);
    this.assert('ok', [element == null], `no element for selector "${selector}"`);
    return this;
  }
  ...
  _domCache () {
    if (this._jsdom instanceof Error) throw this._jsdom;
    if (this._jsdomCache === undefined) this._jsdomCache = new this._jsdom.JSDOM(this.body);
    return this._jsdomCache;
  }

  _prepareTap (tap) {
    this._subtests = [this._assert = tap];
  ...
  async _requestOk (skip, method, url, options) {
    if (this._jsdom === undefined) {
      try {
        this._jsdom = await util.optionalDependency('jsdom');
      } catch (error) {
        this._jsdom = error;
      }
    }
