...
export default class Router extends Route {
  ...
  async dispatch (ctx) {
    ...
    for (let i = 0; i < plan.steps.length; i++) {
      ...
      } else if (plan.steps[i].controller && plan.steps[i].action) {
        const controllerClass = this.controllers[plan.steps[i].controller];
        if (!controllerClass) throw new Error(`Controller ${plan.steps[i].controller} does not exist`);
        const controller = new controllerClass();
        if (!controller[plan.steps[i].action]) throw new Error(`Action ${plan.steps[i].action} does not exist`);
        if (await controller[plan.steps[i].action](ctx) === false) break;
      }
    }
