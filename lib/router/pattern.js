...
class Pattern {
  ...
  match (path, options) {
    ...
    if (!result || (result.remainder.length && result.remainder !== '/')) return null;
    return result.captures;
  }

  matchPartial (path, options = {}) {
    if (!this.regex) this._compile(!!options.isEndpoint);
    const match = path.match(this.regex);
    if (!match) return null;
    const prefix = match.shift();

    const captures = {...this.defaults};
    for (const name of [...this.placeholders, 'format']) {
      if (match.length === 0) break;
      const value = match.shift();
      if (value !== undefined) captures[name] = value;
    }

    return {remainder: path.replace(prefix, ''), captures: captures};
  }
  ...
  render (values = {}, options = {}) {
    ...
    return options.isEndpoint && values.format ? `${path}.${values.format}` : path;
  }
  ...
  _compile (withFormat) {
    ...
    for (const token of this._ast.reverse()) {
      let part = '';
      if (token[0] === OP.slash) {
        ...
        part = util.escapeRegExp(token[1]);
        optional = false;
      } else {
        if (token.length > 2) {
    ...
    if (withFormat) parts.push(this._compileFormat(this.constraints.format, this.defaults.format !== undefined));
    this.regex = new RegExp('^' + parts.join(''), 's');
  }

  _compileFormat (format, withDefault) {
    if (!format) return '';
    const regex = '\\.' + this._compileType(format);
  ...
  _compileType (type) {
    ...
    return '(' + type.sort().reverse().map(val => util.escapeRegExp(val)).join('|') + ')';
  }
