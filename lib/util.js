...
export async function exceptionContext (error, options = {}) {
  const match = error.stack.match(/^\s*at .+ \((.+):(\d+):(\d+)\)$/m);
  ...
  const rl = readline.createInterface({input: file.createReadStream({encoding: 'UTF-8'}), crlfDelay: Infinity});
  let currentLine = 0;
  for await (const line of rl) {
    ...
    context.source.push({line: currentLine, code: line});
  }
