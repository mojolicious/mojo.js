...
export default class Body {
  ...
  async _loadformData () {
    ...
    try {
      await this._receiveFormData();
    } catch (error) {
      if (/maxFileSize exceeded/.test(error.message) !== true) throw error;
      this._files = null;
    }
  }
  ...
  _receiveFormData () {
    return new Promise((resolve, reject) => {
      formidable({maxFileSize: this.maxFileSize, uploadDir: this.uploadDir}).parse(this.raw, (error, fields, files) => {
        if (error != null) {
          reject(error);
          return;
        }
