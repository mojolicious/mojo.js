...
export default class HTTPContext extends Context {
  ...
  redirectTo (target, options = {}) {
    const url = this.urlFor(target, options.values) ?? target;
    this.res.raw.writeHead(options.status ?? 302, {Location: url});
    this.res.raw.end();
  }
  ...
  async render (options, stash) {
    ...
    const result = await this.app.renderer.render(this, options).catch(error => this.helpers.exception(error));
    if (this.res.raw.writableEnded) return;
    ...
    this.res.raw.writeHead(options.status ?? 200, {'Content-Length': length, 'Content-Type': type});
