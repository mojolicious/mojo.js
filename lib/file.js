...
class File {
  ...
  static callerFile () {
    return new File(new StackUtils().capture(3)[2].getFileName());
  }
  ...
  copyFile (newFile, mode) {
    return fsPromises.copyFile(this._path, newFile.toString(), mode);
  }
  ...
  static currentFile () {
    return new File(new StackUtils().capture(2)[1].getFileName());
  }
