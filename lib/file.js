...
export default class File {
  constructor (...parts) {
    this.path = parts.length === 0 ? process.cwd() : parts.length === 1 ? parts[0] : path.join(...parts);
  }
  ...
  sibling (...args) {
    return this.dirname().child(...args);
  }
  ...
  async touch () {
    const now = new Date();
    try {
      await fsPromises.utimes(this.path, now, now);
    } catch (error) {
      await fsPromises.open(this.path, 'w').then(value => value.close());
    }
