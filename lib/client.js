...
export default class Client extends EventEmitter {
  ...
  request (config) {
    ...
    if (typeof config.body === 'string') config.body = Buffer.from(config.body, 'utf8');
    if (config.body instanceof Buffer) config.headers['Content-Length'] = config.body.byteLength;
