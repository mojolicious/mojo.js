...
export default class Renderer {
  ...
  async render (ctx, options) {
    ...
    if (options.jsonPretty !== undefined) {
      return {output: Buffer.from(JSON.stringify(options.jsonPretty, null, 2)), format: options.format ?? 'json'};
    }
    ...
    if (options.view !== undefined) {
      options.include = (options, stash) => ctx.renderToString(options, stash);
