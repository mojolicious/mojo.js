...
function _walk (route, depth, rows, verbose) {
  ...
  // Methods
  row.push(route.methods.length === 0 ? '*' : route.methods.map(method => method.toUpperCase()).join(','));

  // Name
  row.push(route.customName !== undefined ? `"${route.customName}"` : route.defaultName);
